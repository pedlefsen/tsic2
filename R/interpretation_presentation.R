...
plot_iihist <- function(ihist, lb_med_ub, range_start, range_end, 
  ...
  if (FALSE) {
    ...
    iihist <- interpret_ihist(ihist = ihist,
                              ...
                              aggregate_interpreter = aggregate_interpreter, aggregate_interpreter_constructor_fn = aggregate_interpreter_constructor_fn )
...
#    profvis({
    iihist <- interpret_ihist(ihist = ihist,
                              ...
                              aggregate_interpreter = aggregate_interpreter, aggregate_interpreter_constructor_fn = aggregate_interpreter_constructor_fn )
#    })
    agg_interpreter <- aggregate_interpreter_constructor_fn(ihist)
  ...
  iihist <- interpret_ihist(ihist = ihist,
                            ...
                            range_end = range_end,
                            verbose = FALSE,
                            assay_interpreters = assay_interpreters,
                            aggregate_interpreter = aggregate_interpreter, aggregate_interpreter_constructor_fn = aggregate_interpreter_constructor_fn )
  ...
  #x_tick_labels <- format(as.Date(floor(x_breaks), origin = '1970-01-01'), "%B %d")
  x_tick_labels <- format(as.Date(floor(x_breaks), origin = '1970-01-01'), "%B %d")
  ...
  if (!is.null(lb_med_ub) & plot_aggregate){
    vlines_dat <- data.frame(ptid = unique(iihist$ptid),
                             ...
                             result = c('lb', 'med', 'ub'),
                             stringsAsFactors = FALSE)
    # Remove those lines outside plotting range
    if( ( lb_med_ub$lb < range_start ) || ( lb_med_ub$lb > range_end ) ) {
        vlines_dat <- vlines_dat[ vlines_dat$result != "lb", , drop = FALSE ];
    }
    if( ( lb_med_ub$ub < range_start ) || ( lb_med_ub$ub > range_end ) ) {
        vlines_dat <- vlines_dat[ vlines_dat$result != "ub", , drop = FALSE ];
    }
  ...
  if (plot_aggregate){
    ...
    ggplot2::ggplot(iihist_tmp, ggplot2::aes(x = sample_date, y = prob_val, col = result)) +
      ...
      ggplot2::labs(y = paste( 'Probability of observed result given', gsub( " ", "\\\n", event_name ), 'on day indicated by x-axis' )) +
      ggplot2::scale_x_continuous(paste( 'Date of', event_name ), breaks = x_breaks, labels = x_tick_labels) +
    ...
    ggplot2::ggplot(subset(iihist, test_details != 'Aggregate'), ggplot2::aes(x = sample_date, y = prob_val, col = result)) +
      ...
      ggplot2::facet_grid(test_details ~ ., scales = scales) +
      ggplot2::theme(legend.position = 'none') +
      ggplot2::labs(y = paste( "Probability of observed result given\\n", event_name, 'on day indicated by x-axis' )) +
      ggplot2::scale_x_continuous(paste( 'Date of ', event_name ), breaks = x_breaks, labels = x_tick_labels) +
      ggplot2::scale_colour_manual(values = cols)
  }
  if (show_test_dates){

...
biomarker_dynamics <- list(
   ...
   "Ag/Ab" = list(
      ...
      source = "delaney_2017",
      fun = 'weib3_assay_dynamics',
      params = list(location = 0, shape = 3.5, scale = 8) ), # This works, approximately
    "IgG_Supp" = list(
      class = 'IgG_Supp',
      ...
      source = "delaney_2017",
      fun = 'weib3_assay_dynamics',
      params = list(location = 10, shape = 1.733, scale = 5.5) ) # This works, approximately
); # biomarker_dynamics list
...
###################################################
### code chunk number 6: DSHIVInfectionTiming.Rnw:517-702
###################################################
...
unnormalized.density.of.time.of.preceding.event.from.time.of.biomarker.crossing.threshold <- function ( time, shift.low = 0, shift.high = 1, biomarker.dynamics.class = "RNA", the.biomarker.dynamics = biomarker_dynamics[[ biomarker.dynamics.class ]], min.time = get.dynamics.minimum( the.biomarker.dynamics ), max.time = get.dynamics.maximum( the.biomarker.dynamics ), prior.over.shift = function ( shift ) { 1 }, log.results = FALSE, be.verbose = FALSE, abs.tol = 0.0001, rel.tol = 0.0001, subdivisions = 100L ) {
    ...
    if( is.null( prior.over.shift ) ) {
        ...
        .fntointegrate <- Vectorize( function( shift ) { prior.over.shift( shift ) * density.of.time.of.preceding.event.from.time.of.biomarker.crossing.threshold( time, additional.location.shift = shift, log.results = FALSE, the.biomarker.dynamics = the.biomarker.dynamics ) } );
        if( shift.low == shift.high ) {
            # Special case: if this is zero-width, return the density (not zero, since this is used for the posterior density).
...
create.unnormalized.density.of.time.of.preceding.event.from.time.of.biomarker.crossing.threshold.from.uncertainty.interval <- function( low.boundary, high.boundary, biomarker.dynamics.class = "RNA", the.biomarker.dynamics = biomarker_dynamics[[ biomarker.dynamics.class ]], prior.lower.bound = get.dynamics.minimum( the.biomarker.dynamics ), prior.upper.bound = get.dynamics.maximum( the.biomarker.dynamics ), prior.over.biomarker.crossing.threshold.time = function ( time ) { 1 }, log.results = FALSE, be.verbose = FALSE, sample.name = "unnamed sample", abs.tol = 0.0001, rel.tol = 0.0001, subdivisions = 100L ) {
    if( is.null( prior.over.biomarker.crossing.threshold.time ) ) {
        ...
        if( high.boundary == low.boundary ) {
            prior.over.shift <- Vectorize( function( shift ) { if( shift < 0 ) { 0 } else { if( shift > ( high.boundary - low.boundary ) ) { 0 } else { 1 } } } );
        } else {
            prior.over.shift <- Vectorize( function( shift ) { if( shift < 0 ) { 0 } else { if( shift > ( high.boundary - low.boundary ) ) { 0 } else { prior.over.biomarker.crossing.threshold.time( high.boundary - shift ) } } } );
        }
    }
    Vectorize( function( time ) {
        unnormalized.density.of.time.of.preceding.event.from.time.of.biomarker.crossing.threshold(
                   ...
                   min.time = base::max( 0, low.boundary - prior.upper.bound ),
...
###################################################
### code chunk number 7: DSHIVInfectionTiming.Rnw:705-853
###################################################

## Some general-purpose functions.
## Results computation functions
ensure.finite.and.nonzero <- function ( x, .fn, .mutate.x.fn = function( .x ) { .x + 1 }, .stop.cond = x + 1000, .stop.fn = function( x ) { stop( paste( "Stopping because condition is met: x is", x ) ) }, be.verbose = FALSE ) {
...
ensure.finite.and.nonzero.from.above <- function( x, .fn, ... ) { ensure.finite.and.nonzero( x, .fn, .mutate.x.fn = function( .x ) { .x - 1 }, .stop.cond = ( x - 1000 ), ... ) }
...
is.uniform.over.range <- function  ( density.fn, lower, upper, density.maximum = NULL, be.verbose = FALSE, sample.name = "unnamed sample", abs.tol = 0.0001, rel.tol = 0.0001, subdivisions = 100L, ... ) {
    ...
    if( !( ( density.fn.result.lower > 0 ) && ( density.fn.result.upper > 0 ) ) ) {
        ...
        new.lower <-
            ensure.finite.and.nonzero.from.below( lower, density.fn, be.verbose = be.verbose, .stop.fn = function( x ) { -Inf } );
        if( !is.finite( new.lower ) ) {
            ## Can't find a nonzero value above "lower" so it's uniform enough.
            return( TRUE );
        }
        if( new.lower != lower ) {
            lower <- new.lower;
        ...
        new.upper <-
            ensure.finite.and.nonzero.from.above( upper, density.fn, be.verbose = be.verbose, .stop.fn = function( x ) { Inf } );
        if( !is.finite( new.upper ) ) {
            ## Can't find a nonzero value below "upper" so it's uniform enough.
            return( TRUE );
        }
        if( new.upper != upper ) {
            upper <- new.upper;
...
###################################################
### code chunk number 8: DSHIVInfectionTiming.Rnw:859-874
###################################################
...
create.biomarker.normalized.posterior.density <- function ( low.boundary = 0, high.boundary = 1, posterior.maximum = NULL, biomarker.dynamics.class = "RNA", prior.lower.bound = low.boundary - get.dynamics.maximum( the.biomarker.dynamics ), prior.upper.bound = high.boundary - get.dynamics.minimum( the.biomarker.dynamics ), be.verbose = FALSE, sample.name = "unnamed sample"
                                                           , the.biomarker.dynamics = biomarker_dynamics[[ biomarker.dynamics.class ]], abs.tol = 0.0001, rel.tol = 0.0001, subdivisions = 100L
                                                           , ... ) {
    create.normalized.posterior.density( posterior.fn.creator.fn = create.unnormalized.density.of.time.of.preceding.event.from.time.of.biomarker.crossing.threshold.from.uncertainty.interval, low.boundary = low.boundary
                                        , high.boundary = high.boundary, posterior.maximum = posterior.maximum, the.biomarker.dynamics = the.biomarker.dynamics, prior.lower.bound = prior.lower.bound, prior.upper.bound = prior.upper.bound, be.verbose = be.verbose, sample.name = sample.name, rel.tol = rel.tol, abs.tol = abs.tol, subdivisions = subdivisions
                                        , ... )
...
###################################################
### code chunk number 9: DSHIVInfectionTiming.Rnw:877-1171
###################################################
...
ihist.RNAonly.oneday31 <- data.frame(
  ptid = 'RNA-only',
  #sample_date = c(18000.5, 18031.5),
  sample_date = c(18030.5, 18031.5),
  test = c('abbott_real_time_weib3_delaney_and_manufacturer', 'abbott_real_time_weib3_delaney_and_manufacturer' ),
  result = c('-', '+'),
  stringsAsFactors = FALSE )
...
ihist.Comboonly.oneday6 <- data.frame(
  ptid = 'Combo-only',
  sample_date = c(18006.5, 18007.5),
...
ihist.Comboonly.31days <- data.frame(
  ptid = 'Combo-only',
  sample_date = c(18006.5, 18037.5),
  test = c('architect_weib3_delaney', 'architect_weib3_delaney' ),
  result = c('-', '+'),
  stringsAsFactors = FALSE )

ihist.Comboonly.oneday6.withunhelpfulRNA <- data.frame(
  ptid = 'Combo-only',
  sample_date = c(18006.5, 18006.5, 18007.5),
...
ihist.ComboRNA.oneday6.oneday1 <- data.frame(
  ptid = 'Combo-RNA',
  sample_date = c(18000.5, 18001.5, 18006.5, 18007.5),
...
## These have just Ab
ihist.Abonly.oneday21 <- data.frame(
  ptid = 'Ab-only',
  sample_date = c(18021.5, 18022.5),
...
## If return.list.with.inputs is TRUE, the first element of the returned list is the key/value pair for key "normalized.posterior.density.of.infection.causing.exposure.time", so you can access it as either rv[[1]] or rv[["normalized.posterior.density.of.infection.causing.exposure.time"]]. The rest of the list contains the arguments to this function (on its final call, since it is recursive).
create.normalized.posterior.density.of.infection.causing.exposure.time <- function ( remaining.biomarker.bounds.to.process.last.to.first, prior.lower.bound = 0, prior.upper.bound = as.numeric( Sys.Date() ), prior.over.last.biomarker.crossing.threshold.time = function ( time ) { 1 }, return.list.with.inputs = FALSE, sample.name = "unnamed sample", ... ) {
    ### Strategy: Take the bottom one off, use it to construct a posterior, used as a prior; tail recurse. Base case: RNA.

    ## Define the biomarker.
    if( length( remaining.biomarker.bounds.to.process.last.to.first ) == 1 ) {
        ...
        # pop it off the tail end of the stack. (In perl this would be "pop")
        the.biomarker <-
            names( remaining.biomarker.bounds.to.process.last.to.first )[ length( remaining.biomarker.bounds.to.process.last.to.first ) ];
    }
    ...
    high.boundary <-
        remaining.biomarker.bounds.to.process.last.to.first[[ the.biomarker ]][ 2, "adjusted.date" ];
    ...
    if( is.null( prior.over.last.biomarker.crossing.threshold.time ) ) {
        ...
        prior.over.last.biomarker.crossing.threshold.time.is.uniform <- 
            is.uniform.over.range( prior.over.last.biomarker.crossing.threshold.time, low.boundary, high.boundary );
        if( prior.over.last.biomarker.crossing.threshold.time.is.uniform ) {
            if( FALSE ) { #be.verbose ) {
                cat( "Replacing the given prior with a uniform prior for efficiency as it is effectively uniform", fill = TRUE );
            }
        }
    }
    if( prior.over.last.biomarker.crossing.threshold.time.is.uniform ) {
    ...
    ## Base case:
    if( length( remaining.biomarker.bounds.to.process.last.to.first ) == 1 ) {
        normalized.posterior.density.of.infection.causing.exposure.time <- create.biomarker.normalized.posterior.density( low.boundary = low.boundary, high.boundary = high.boundary, prior.lower.bound = prior.lower.bound, prior.upper.bound = prior.upper.bound
                                                                                                                         , posterior.maximum = NULL, biomarker.dynamics.class = "RNA", prior.over.biomarker.crossing.threshold.time = prior.over.last.biomarker.crossing.threshold.time, sample.name = paste( the.biomarker, remaining.biomarker.bounds.to.process.last.to.first[[ the.biomarker ]][ 1, "ptid" ] )
                    , ... );
        if( return.list.with.inputs ) {
            return(
                   list(
                        normalized.posterior.density.of.infection.causing.exposure.time = normalized.posterior.density.of.infection.causing.exposure.time,
                        remaining.biomarker.bounds.to.process.last.to.first = remaining.biomarker.bounds.to.process.last.to.first,
                        prior.lower.bound = prior.lower.bound,
                        prior.upper.bound = prior.upper.bound,
                        prior.over.last.biomarker.crossing.threshold.time = prior.over.last.biomarker.crossing.threshold.time,
                        sample.name = sample.name,
                        ...
                        )
                   );
        } else {
            return( normalized.posterior.density.of.infection.causing.exposure.time );
        } # End if return.list.with.inputs .. else ..
    } # End if this is the base case, return.
    ...
    posterior.for.preceding.event <-
        create.biomarker.normalized.posterior.density( low.boundary = low.boundary, high.boundary = high.boundary
                                                      , posterior.maximum = NULL, biomarker.dynamics.class = the.biomarker, prior.over.biomarker.crossing.threshold.time = prior.over.last.biomarker.crossing.threshold.time, sample.name = paste( the.biomarker, remaining.biomarker.bounds.to.process.last.to.first[[ the.biomarker ]][ 1, "ptid" ] )
        , ... );
    ...
    remaining.biomarker.bounds.to.process.last.to.first.backup <-
        remaining.biomarker.bounds.to.process.last.to.first;
    prior.over.last.biomarker.crossing.threshold.time.backup <-
        prior.over.last.biomarker.crossing.threshold.time;
    return( create.normalized.posterior.density.of.infection.causing.exposure.time( remaining.biomarker.bounds.to.process.last.to.first = remaining.biomarker.bounds.to.process.last.to.first.excluding.this.one, prior.lower.bound = prior.lower.bound, prior.upper.bound = prior.upper.bound, prior.over.last.biomarker.crossing.threshold.time = posterior.for.preceding.event, return.list.with.inputs = return.list.with.inputs, sample.name = sample.name, ... ) );
...
construct.aggregate.interpreter <- function ( ihist, prior.lower.bound = -Inf, prior.upper.bound = Inf, lower.bound.offset.from.first.positive = -100, minimum.bound.width = 150, be.verbose = FALSE, ... ) {
    ...
    ## We process these from latest to earliest. The adjusted.date values bound the time at which each biomarker crossed its threshold. So we start from the latest, integrating over that to get the prior for the next one, etc. That's what this function does:
    return( create.normalized.posterior.density.of.infection.causing.exposure.time( remaining.biomarker.bounds.to.process.last.to.first = ihist.most.informative.by.biomarker, prior.over.last.biomarker.crossing.threshold.time = NULL, prior.lower.bound = prior.lower.bound, prior.upper.bound = prior.upper.bound, be.verbose = be.verbose, ... ) );
} # construct.aggregate.interpreter ( ihist )



###################################################
### code chunk number 10: DSHIVInfectionTiming.Rnw:1174-1363
###################################################
...
estimate.lb.med.ub.for.density.fun <- function ( fun, range_start, range_end, verbose = FALSE, label = 'unlabeled', 
    ...
    return( list( lb = unname( desired.quantiles[ '0.025' ] ), med = unname( desired.quantiles[ '0.5' ] ), ub = unname( desired.quantiles[ '0.975' ] ), aoc = 1, extra_tiles = extra_tiles, extra_computed_tiles = extra.computed.quantiles, date_splits = date_splits, aoc_left_of_date = aoc_left_of_date ) );
} # estimate.lb.med.ub.for.density.fun (..)



###################################################
### code chunk number 11: DSHIVInfectionTiming.Rnw:1367-1371
###################################################
...
###################################################
### code chunk number 12: DSHIVInfectionTiming.Rnw:1375-1378
###################################################
...
###################################################
### code chunk number 13: DSHIVInfectionTiming.Rnw:1382-1440
###################################################
##ENDCOPYMARK
